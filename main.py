...
elif isinstance(playlist, list): #Проверяем, является ли playlist списком
        for song in selected_songs:
            duration = playlist_b[song]
            #Преобразуем время из float в строку перед разделением
...
# Пример использования функции с плейлистом playlist_c, выбраны 5 песен тут меняйте как хотите
duration_c = get_duration(playlist_c, 5)
print(f"Общее время звучания 5 случайных песен из playlist_c: {duration_c}")

# Пример использования функции с плейлистом playlist_b, выбраны 3 все тож самое 
